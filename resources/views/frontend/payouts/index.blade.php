        ...
        <div class="col-md-12">
            @can('payout_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.payouts.create') }}">
                            ...
                            <tbody>
                                @foreach($payouts as $key => $payout)
                                    <tr data-entry-id="{{ $payout->id }}">
                                        ...
                                        <td>
                                            @can('payout_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.payouts.show', $payout->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('payout_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.payouts.edit', $payout->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('payout_delete')
                                                <form action="{{ route('frontend.payouts.destroy', $payout->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('payout_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.payouts.massDestroy') }}",
